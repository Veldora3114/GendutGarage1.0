# 🐳 Docker untuk Anak Kampus — Satu PC Lab, 40 Mahasiswa, Nol Bentrok (Studi Kasus: Gendut Garage)
...
* Kamu pakai **Flutter 3.35**, teman sebelah pakai **Flutter 3.19** karena dependensi lamanya belum dukung Dart 3.
...
* Belum lagi waktu instalasinya: unduh Flutter SDK 1.5 GB, unduh Android Studio / Chrome driver, jalankan `pub get`, tunggu kompilasi. Satu orang habis 45 menit kalau lancar. Kali 40 orang? Itu hampir 30 jam, dan lab kampus keburu tutup.

Docker menyelesaikan masalah itu dalam satu pukulan telak. Artikel/tutorial ini dibuat khusus untuk memandu kamu memasang aplikasi **Gendut Garage** (atau aplikasi Flutter Web lainnya) di PC lab kampus tanpa bentrok, tanpa ribet, dan selesai dalam hitungan detik.
...
Di proyek `GendutGarage1.0`, kita sudah siapkan dua berkas utama:
...
### Jalur A: Ekspor File `.tar` Pakai Flashdisk (Paling Disukai, 100% Offline)
...
3. **Di PC Lab Kampus:**
   Colok flashdisk, buka Command Prompt / PowerShell:
   ```bash
...
Jika laptop kamu RAM-nya cuma 4GB atau tidak kuat pasang Docker Desktop, biarkan **server gratisan milik GitHub** yang membuatkan Docker Image-nya untukmu!

Kamu cukup buat file `.github/workflows/docker-build.yml` di repository-mu:
```yaml
    ...
    steps:
      ...
      - name: Build and Push Docker Image
        ...
        with:
          ...
          push: true
          tags: ${{ secrets.DOCKERHUB_USERNAME }}/gendut-garage:latest
```
Setiap kali kamu `git push origin main`, GitHub akan otomatis mengompilasi dan mengunggah image ke Docker Hub.  
...
Bagi anak kampus, menguasai Docker bukan cuma menyelamatkan nilai skripsi kamu dari drama *"di laptop saya jalan, Pak"*, tapi juga jadi portofolio mahal waktu melamar kerja nanti. Tim engineering di industri menyukai orang yang paham kontainerisasi karena aplikasi mereka langsung siap jalan di server mana pun.

Selamat mencoba, pasang port NIM-mu dengan bangga, dan sukses buat sidangnya! 🚀
